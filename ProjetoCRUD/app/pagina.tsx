...
const Listagem = () => {
  ...
  const excluirPessoa = (id: string | undefined) => {
    firebase.firestore().collection('Pessoas').doc(id).delete();
  }

  const atualizarPessoa = (id: string | undefined, dados: { [x: string]: any; nome?: string; sobrenome?: string; cpf?: string; }) => {
    firebase.firestore().collection('Pessoas').doc(id).update(dados);
    closeEdit();
  }

  const showEdit = (id: SetStateAction<string>) => {
    setEditState("flex")
    setEditId(id)
  }
